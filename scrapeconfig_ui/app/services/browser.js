...
/* jshint ignore:start */
const META_STYLE = `<style title="scrapeconfig-show-meta">
    head {
        display: block;
        display: -webkit-flex;
        display: flex;
    ...
    link[href][rel]::after {
        content: 'Link: rel: "' attr(rel) '" href: "' attr(data-scrapeconfig-href) '"';
    }
    link[href][rel][data-scrapeconfig-hidden-media]::after {
        content: 'Link: rel: "' attr(rel) '" href: "' attr(data-scrapeconfig-href) '" media: "' attr(data-scrapeconfig-hidden-media) '"';
    }
    link[href][rel][type]::after {
        content: 'Link: rel: "' attr(rel) '" href: "' attr(data-scrapeconfig-href) '" type: "' attr(type) '"';
    }
    link[href][rel][type][data-scrapeconfig-hidden-media]::after {
        content: 'Link: rel: "' attr(rel) '" href: "' attr(data-scrapeconfig-href) '" type: "' attr(type) '" media: "' attr(data-scrapeconfig-hidden-media) '"';
    }
</style>`;
/* jshint ignore:end */
...
export default Ember.Service.extend(Ember.Evented, {
    ...
    checkCSS() {
        ...
        const $showMetaStyleElement = $iframe.find('style[title="scrapeconfig-show-meta"]');
    ...
    disableCSS() {
        ...
        if (this.get('cssEnabled') && iframe) {
            ...
            const $styles = $iframe.find(
                'style:not([title="scrapeconfig-show-meta"]), link[rel="stylesheet"]');
            renameAttr($styles, 'media', 'data-scrapeconfig-hidden-media');
            // disable stylesheets using an impossible media query
            $styles.attr('media', '(width: -1px)');
            renameAttr($iframe.find('[style]'), 'style', 'data-scrapeconfig-hidden-style');
    ...
    enableCSS() {
        ...
        if (!this.get('cssEnabled') && iframe) {
            const $iframe = this.get('$document');
            $iframe.find('style[title="scrapeconfig-show-meta"]').remove();
            const $styles = $iframe.find(
                'style:not([title="scrapeconfig-show-meta"]), link[rel="stylesheet"]');
            $styles.attr('media', null);
            renameAttr($styles, 'data-scrapeconfig-hidden-media', 'media');
            renameAttr($iframe.find('[data-scrapeconfig-hidden-style]'),
                                    'data-scrapeconfig-hidden-style', 'style');
