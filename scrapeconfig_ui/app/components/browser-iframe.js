...
const BrowserIFrame = Ember.Component.extend({
    ...
    _loadUrl() {
        ...
        if (failures && ((new Date() - new Date(failures.dt))/(1000*3600) < 1)) {
            ...
            if (failed > 3 && window.navigator.onLine) {
                // Tell user that page is not working with ScrapeConfig
    ...
    unbindEventHandlers() {
        $(this.element.contentDocument).off('.scrapeconfig-iframe');
    ...
    bindEventHandlers() {
        ...
        $iframe.on(
            ['keyup', 'keydown', 'keypress', 'input',  'mousedown', 'mouseup'].map(
                eventName => `${eventName}.scrapeconfig.scrapeconfig-iframe`).join(' '),
            e => {
        ...
        $iframe.on('input.scrapeconfig.scrapeconfig-iframe', e => {
            if (this.get('browser.mode') === NAVIGATION_MODE) {
                this.postEvent(e);
            } 
        ...
        */
        ...
        $iframe.on('click.scrapeconfig.scrapeconfig-iframe', e => {
            if (this.get('browser.mode') === NAVIGATION_MODE) {
        ...
        $iframe.on('change.scrapeconfig.scrapeconfig-iframe', e => {
