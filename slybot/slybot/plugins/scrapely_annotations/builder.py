...
def _clean_annotation_data(data):
    ...
    for ann in data:
        if ann.get('item_container'):
            ann['annotations'] = {'#scrapeconfig-content': '#dummy'}
            ann['text-content'] = '#scrapeconfig-content'
        elif 'data' in ann:
...
def add_repeated_field(annotation, elems, page):
    ...
    if len(parent):
        ...
        parent_annotation = {
            'item_container': True,
            'id': container_id,
            'annotations': {'#scrapeconfig-content': '#dummy'},
            'text-content': '#scrapeconfig-content',
