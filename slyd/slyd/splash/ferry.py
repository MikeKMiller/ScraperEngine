...
from scrapeconfig_api.errors import BaseHTTPError
...
from scrapeconfig_orm.models import Project
from scrapeconfig_orm.datastore import data_store_context
from scrapeconfig_orm.utils import short_guid
from scrapeconfig_api.utils.spiders import load_spider_data

from .qtutils import QObject, pyqtSlot, QWebElement
from .cookies import ScrapeConfigCookieJar
...
class ScrapeConfigNetworkManager(SplashQNetworkAccessManager):
    _raw_html = None

    def createRequest(self, operation, request, outgoingData=None):
        reply = super(ScrapeConfigNetworkManager, self).createRequest(
...
class ScrapeConfigBrowserTab(BrowserTab):
    @property
    def url(self):
        """ Current URL """
        if self._closing:
    ...
    def evaljs(self, *args, **kwargs):
        return super(ScrapeConfigBrowserTab, self).evaljs(*args, **kwargs)
...
class ScrapeConfigJSApi(QObject):
    def __init__(self, protocol):
        super(ScrapeConfigJSApi, self).__init__()
...
def blacklist_error(data, socket):
    ...
    msg = "Sorry ScrapeConfig doesn't support this page :("
...
class FerryServerProtocol(WebSocketServerProtocol):
    ...
    def getElementByNodeId(self, nodeid):
        self.tab.web_page.mainFrame().evaluateJavaScript(
            'liveScrapeConfigPage.pyGetByNodeId(%s)' % nodeid
        )
    ...
    def open_tab(self, meta=None):
        ...
        self.factory[self].tab = ScrapeConfigBrowserTab(
        ...
        cookiejar = ScrapeConfigCookieJar(self.tab.web_page, self)
        ...
        self.js_api = ScrapeConfigJSApi(self)
    ...
    def populate_window_object(self):
        main_frame = self.tab.web_page.mainFrame()
        main_frame.addToJavaScriptWindowObject('__scrapeconfigApi', self.js_api)
    ...
    def open_spider(self, meta, storage=None, project=None):
        ...
        if not self.settings.get('SPLASH_URL'):
            self.settings.set('SPLASH_URL', 'scrapeconfig')
