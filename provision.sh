...
install_python_deps(){
    ...
    pip install -r "$APP_ROOT/scrapeconfig_server/requirements.txt"
...
configure_initctl(){
    cp "$APP_ROOT/scrapeconfig.conf" /etc/init
    sed 's/\/app\//'""${APP_ROOT//\//\\\/}""'\//g' -i /etc/init/scrapeconfig.conf
}
...
migrate_django_db(){
    cd "$APP_ROOT/scrapeconfig_server"
    python manage.py makemigrations scrapeconfig_dashboard
